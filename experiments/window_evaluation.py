"""Window Evaluation: Plain, Generated, Navigator, and Navigator + Window.

The experiment reuses the existing collectors, comparator, scorer, validator,
and metrics helpers. It writes an aggregate report under ``exp_xxx/window``.
The Window profile is derived from the immutable real-browser baseline and is
injected only for the Navigator + Window mode.
"""
...
log = setup_logging("experiment.window_evaluation")
MODE_ORDER = ("plain", "generated", "navigator", "navigator_window")
MODE_LABELS = {
    "plain": "Plain",
    "generated": "Generated",
    "navigator": "Navigator",
...
def _window_profile(baseline: Baseline) -> dict[str, Any]:
    """Build a coherent, data-driven Window profile from the baseline."""
    source = baseline.fingerprint.get("window") or {}
    profile = {
        key: value
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if mode == "plain":
                ...
                if mode == "navigator_window":
                    profile = json.dumps(
                        _window_profile(settings.baseline),
                        ensure_ascii=False,
                        separators=(",", ":"),
                    )
                    page.add_init_script(
                        "globalThis.__stealth=globalThis.__stealth||{};"
                        f"globalThis.__stealth.windowProfile={profile};"
                    )
                apply_generated(page)
                if mode in {"navigator", "navigator_window"}:
    ...
    return Collected(
        ...
        document={
            "_meta": {
                ...
                "navigator_module_applied": mode in {"navigator", "navigator_window"},
                "window_module_applied": mode == "navigator_window",
                "window_profile_source": "baseline.window" if mode == "navigator_window" else None,
...
def _mode_result(
    ...
    summary = {
        ...
        "metrics": {
            ...
            "regressed": len(metrics.regressed_keys),
            "unchanged": len(metrics.unchanged_keys),
            "improved_keys": metrics.improved_keys,
            "regressed_keys": metrics.regressed_keys,
...
def _conclusion(rows: dict[str, dict[str, Any]]) -> str:
    navigator = rows["navigator"]["summary"]
    combined = rows["navigator_window"]["summary"]
    n_scores = navigator["scores"]
    c_scores = combined["scores"]
    improvements = sum(
        float(c_scores.get(key) or 0) > float(n_scores.get(key) or 0)
        for key in ("overall_similarity", "weighted_cf_score", "navigator_category_score", "window_category_score")
    )
    regressions = int(combined["metrics"]["regressed"])
    if regressions == 0 and improvements >= 2:
        return "Navigator + Window meningkatkan fingerprint dibanding Navigator saja tanpa regression terukur."
    if regressions > 0:
        return f"Navigator + Window menghasilkan {regressions} regression; Window perlu diaudit sebelum dianggap aman."
    return "Navigator + Window belum menunjukkan peningkatan yang cukup dibanding Navigator saja."
...
def render_report(rows: dict[str, dict[str, Any]], output: Path) -> str:
    ...
    for mode in MODE_ORDER:
        ...
        lines.append(
            ...
            f"{_number(scores['weighted_cf_score'])} | {_number(scores['window_category_score'])} | "
            f"{_number(scores['navigator_category_score'])} | {metrics['total_diff']} | "
            f"{metrics['improved']} | {metrics['regressed']} |"
        )
    lines.extend(["", "## Conclusion", "", _conclusion(rows), ""])
    return "\n".join(lines)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Window Evaluation across four stealth modes.")
    ...
    parser.add_argument("--label", default="window-evaluation")
...
def run(settings: Settings) -> int:
    ...
    output = experiment.directory / "window"
    ...
    write_text_exclusive(output / "window_report.md", render_report(results, output))
