"""Experiment 009: evaluate the Permissions API compatibility module."""
...
log = setup_logging("experiment.permissions_evaluation")
MODE_ORDER = (
    "plain",
    "generated",
    "navigator",
...
def _permissions_profile(baseline: Baseline) -> dict[str, Any]:
    source = baseline.fingerprint.get("permissions")
    return dict(source) if isinstance(source, dict) else {}
...
def collect_mode(settings: Settings, mode: str) -> Collected:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if mode == "plain":
                ...
                if mode in {"navigator_window", "navigator_window_screen", "navigator_window_screen_chrome", "navigator_window_screen_chrome_permissions"}:
                    profiles["windowProfile"] = _window_profile(settings.baseline)
                if mode in {"navigator_window_screen", "navigator_window_screen_chrome", "navigator_window_screen_chrome_permissions"}:
                    profiles["screenProfile"] = _screen_profile(settings.baseline)
                if mode in {"navigator_window_screen_chrome", "navigator_window_screen_chrome_permissions"}:
                    profiles["chromeProfile"] = _chrome_profile(settings.baseline)
                if mode == "navigator_window_screen_chrome_permissions":
                    profiles["permissionsProfile"] = _permissions_profile(settings.baseline)
                if profiles:
                ...
                if mode in {"navigator", "navigator_window", "navigator_window_screen", "navigator_window_screen_chrome", "navigator_window_screen_chrome_permissions"}:
    ...
    return Collected(
        ...
        document={
            "_meta": {
                "experiment": "Experiment 009 - Permissions Evaluation",
                "mode": mode,
                ...
                "navigator_module_applied": mode != "plain" and mode != "generated",
                "window_module_applied": mode in {"navigator_window", "navigator_window_screen", "navigator_window_screen_chrome", "navigator_window_screen_chrome_permissions"},
                "screen_module_applied": mode in {"navigator_window_screen", "navigator_window_screen_chrome", "navigator_window_screen_chrome_permissions"},
                "chrome_module_applied": mode in {"navigator_window_screen_chrome", "navigator_window_screen_chrome_permissions"},
                "permissions_module_applied": mode == "navigator_window_screen_chrome_permissions",
                "window_profile_source": "baseline.window" if mode in {"navigator_window", "navigator_window_screen", "navigator_window_screen_chrome", "navigator_window_screen_chrome_permissions"} else None,
                "screen_profile_source": "baseline.screen" if mode in {"navigator_window_screen", "navigator_window_screen_chrome", "navigator_window_screen_chrome_permissions"} else None,
                "chrome_profile_source": "baseline.chrome" if mode in {"navigator_window_screen_chrome", "navigator_window_screen_chrome_permissions"} else None,
                "permissions_profile_source": "baseline.permissions" if mode == "navigator_window_screen_chrome_permissions" else None,
...
def _conclusion(rows: dict[str, dict[str, Any]]) -> str:
    before = rows["navigator_window_screen_chrome"]["summary"]
    after = rows["navigator_window_screen_chrome_permissions"]["summary"]
    b = before["scores"]
    a = after["scores"]
    improvements = sum(
        float(a.get(key) or 0) > float(b.get(key) or 0)
        for key in ("overall_similarity", "weighted_cf_score", "permissions_category_score", "chrome_category_score", "navigator_category_score", "window_category_score", "screen_category_score")
    )
    regressions = int(after["metrics"]["regressed"])
    permissions_score = float(a.get("permissions_category_score") or 0)
    if regressions == 0 and improvements >= 2 and permissions_score >= 90:
        return "Permissions Module meningkatkan fingerprint dibanding Chrome stack tanpa stable regression."
    if regressions:
        return f"Permissions Module menghasilkan {regressions} stable regression; perlu diaudit lebih lanjut."
    return "Permissions Module belum menunjukkan peningkatan yang cukup dibanding Chrome stack."
...
def render_report(rows: dict[str, dict[str, Any]], output: Path) -> str:
    ...
    for mode in MODE_ORDER:
        ...
        lines.append(
            ...
            f"{_number(scores['permissions_category_score'])} | {_number(scores['chrome_category_score'])} | "
            f"{_number(scores['navigator_category_score'])} | {_number(scores['window_category_score'])} | "
            f"{_number(scores['screen_category_score'])} | {metrics['total_diff']} | {metrics['improved']} | {metrics['regressed']} |"
        )
    lines.extend(["", "Environmental note: network-sampled Navigator connection values are listed separately and excluded from stable regression counts.", "", "## Conclusion", "", _conclusion(rows), ""])
    return "\n".join(lines)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Experiment 009: Permissions Evaluation.")
    ...
    parser.add_argument("--label", default="permissions-evaluation")
...
def run(settings: Settings) -> int:
    ...
    write_text_exclusive(output / "permissions_report.md", report)
    print(report)
    return 0
