"""Screen Evaluation: compare the stealth stack as Screen is added.

This is an experiment consumer of the existing collectors, comparator, scorer,
validator, and metrics helpers.  Every mode runs in a fresh browser context so
the observations are independent and the final report is write-once under a
new ``reports/experiments/exp_xxx/screen`` directory.
"""
...
log = setup_logging("experiment.screen_evaluation")
MODE_ORDER = ("plain", "generated", "navigator", "navigator_window", "navigator_window_screen")
MODE_LABELS = {
    "plain": "Plain",
    "generated": "Generated",
    "navigator": "Navigator",
...
# Chromium samples these values from the host/network at navigation time.  A
# change between two clean browser launches is not caused by the Screen
# module, so it must not be reported as a module regression.  Keep this list
# deliberately narrow; all stable Navigator/Window/Screen properties remain
# eligible for regression detection.
ENVIRONMENT_DYNAMIC_KEYS = frozenset({
...
def _profile_values(source: Any) -> dict[str, Any]:
    return dict(source) if isinstance(source, dict) else {}


def _window_profile(baseline: Baseline) -> dict[str, Any]:
    """Create the same coherent Window profile used by Window Evaluation."""
    source = _profile_values(baseline.fingerprint.get("window"))
    profile = {key: value for key, value in source.items() if value is not None}
    profile["visualViewport"] = {
        "width": profile.get("innerWidth"),
        "height": profile.get("innerHeight"),
        "scale": profile.get("devicePixelRatio"),
        "offsetLeft": 0,
        "offsetTop": 0,
        "pageLeft": profile.get("scrollX", 0),
        "pageTop": profile.get("scrollY", 0),
    }
    return profile


def _screen_profile(baseline: Baseline) -> dict[str, Any]:
    """Use the real-browser Screen observation as a data-driven profile."""
    source = _profile_values(baseline.fingerprint.get("screen"))
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if mode == "plain":
                ...
                if mode in {"navigator_window", "navigator_window_screen"}:
                    profile_values["windowProfile"] = _window_profile(settings.baseline)
                if mode == "navigator_window_screen":
                    profile_values["screenProfile"] = _screen_profile(settings.baseline)
                if profile_values:
                    serialized = json.dumps(
                        profile_values, ensure_ascii=False, separators=(",", ":")
                    )
                ...
                if mode in {"navigator", "navigator_window", "navigator_window_screen"}:
    ...
    return Collected(
        ...
        document={
            "_meta": {
                ...
                "navigator_module_applied": mode in {"navigator", "navigator_window", "navigator_window_screen"},
                "window_module_applied": mode in {"navigator_window", "navigator_window_screen"},
                "screen_module_applied": mode == "navigator_window_screen",
                "window_profile_source": "baseline.window" if mode in {"navigator_window", "navigator_window_screen"} else None,
                "screen_profile_source": "baseline.screen" if mode == "navigator_window_screen" else None,
...
def _reported_regressions(keys: list[str]) -> tuple[list[str], list[str]]:
    """Separate stable regressions from host-sampled environmental changes."""
    stable = [key for key in keys if key not in ENVIRONMENT_DYNAMIC_KEYS]
    environmental = [key for key in keys if key in ENVIRONMENT_DYNAMIC_KEYS]
    return stable, environmental
...
def _mode_result(
    ...
    reported_regressions, environmental_regressions = _reported_regressions(
        metrics.regressed_keys
    )
    # Keep the validator output useful while making the report's regression
    # count represent stable properties controlled by the modules under test.
...
def _conclusion(rows: dict[str, dict[str, Any]]) -> str:
    before = rows["navigator_window"]["summary"]
    after = rows["navigator_window_screen"]["summary"]
    ...
    screen_score = after_scores.get("screen_category_score")
    if regressions == 0 and improvements >= 2 and (screen_score or 0) >= (before_scores.get("screen_category_score") or 0):
        return "Navigator + Window + Screen meningkatkan fingerprint dibanding Navigator + Window tanpa regression terukur."
    if regressions:
        return f"Screen menghasilkan {regressions} regression; module perlu diaudit sebelum dianggap aman."
    return "Screen belum menunjukkan peningkatan yang cukup dibanding Navigator + Window."
...
def render_report(rows: dict[str, dict[str, Any]], output: Path) -> str:
    ...
    for mode in MODE_ORDER:
        ...
        lines.append(
            ...
            f"{_number(scores['weighted_cf_score'])} | {_number(scores['screen_category_score'])} | "
            f"{_number(scores['window_category_score'])} | {_number(scores['navigator_category_score'])} | "
    ...
    lines.extend(
        [
            ...
            "listed separately and excluded from module regression counts.",
            "",
            "## Conclusion",
            "",
            _conclusion(rows),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Screen Evaluation across five stealth modes.")
    ...
    parser.add_argument("--label", default="screen-evaluation")
...
def run(settings: Settings) -> int:
    ...
    write_text_exclusive(output / "screen_report.md", render_report(results, output))
    print(render_report(results, output))
    return 0
