"""Experiment 011: evaluate the Speech Synthesis compatibility module."""
...
log = setup_logging("experiment.speech_evaluation")

# The two descriptive stack rows are intentionally retained: existing
# dashboards and reports use both names when comparing Chrome milestones.
MODE_ORDER = (
    "plain",
    "generated",
    "navigator",
...
MODE_LABELS = {
    ...
    "permissions_fonts": "Permissions + Fonts",
    "fonts_speech": "Fonts + Speech",
}
...
def _profile_for_mode(baseline: Baseline, mode: str) -> dict[str, Any]:
    profiles: dict[str, Any] = {}
    stack = {
        "navigator_window", "navigator_window_screen", "navigator_window_screen_chrome",
        "chrome_stack", "chrome_permissions", "permissions_fonts", "fonts_speech",
    }
    if mode in stack:
        profiles["windowProfile"] = _section(baseline, "window")
    if mode in {
        "navigator_window_screen", "navigator_window_screen_chrome", "chrome_stack",
        "chrome_permissions", "permissions_fonts", "fonts_speech",
    }:
        profiles["screenProfile"] = _section(baseline, "screen")
    if mode in {
        "navigator_window_screen_chrome", "chrome_stack", "chrome_permissions",
        "permissions_fonts", "fonts_speech",
    }:
        profiles["chromeProfile"] = _section(baseline, "chrome")
    if mode in {"chrome_permissions", "permissions_fonts", "fonts_speech"}:
        profiles["permissionsProfile"] = _section(baseline, "permissions")
    if mode in {"permissions_fonts", "fonts_speech"}:
        profiles["fontProfile"] = _section(baseline, "fonts")
    if mode == "fonts_speech":
        profiles["speechProfile"] = _speech_profile(baseline)
    return profiles


def _modules_for_mode(mode: str) -> list[str]:
...
def collect_mode(settings: Settings, mode: str) -> Collected:
    ...
    modules = _modules_for_mode(mode)
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if mode == "plain":
                ...
                profiles = _profile_for_mode(settings.baseline, mode)
                if profiles:
    ...
    return Collected(
        ...
        document={
            "_meta": {
                "experiment": "Experiment 011 - Speech Evaluation",
                "mode": mode,
                ...
                "speech_profile_source": "baseline.speech" if mode == "fonts_speech" else None,
...
def _conclusion(rows: dict[str, dict[str, Any]]) -> str:
    before = rows["permissions_fonts"]["summary"]
    after = rows["fonts_speech"]["summary"]
    b, a = before["scores"], after["scores"]
    regressions = int(after["metrics"]["regressed"])
    speech_before = float(b.get("speech_category_score") or 0)
    speech_after = float(a.get("speech_category_score") or 0)
    ...
    if regressions:
        return f"Speech Module menghasilkan {regressions} stable regression; perlu diaudit lebih lanjut."
    if speech_after > speech_before and overall_after > overall_before and cf_after > cf_before and diff_reduced:
        return "Speech Module meningkatkan fingerprint secara terukur tanpa stable regression."
    if speech_after >= 90 and speech_after == speech_before:
        return "Speech category sudah selaras dengan baseline; module mempertahankan native behavior tanpa stable regression."
    return "Speech Module belum menunjukkan peningkatan yang cukup dibanding Permissions + Fonts stack."
...
def render_report(rows: dict[str, dict[str, Any]], output: Path) -> str:
    ...
    for mode in MODE_ORDER:
        ...
        lines.append(
            ...
            f"{_number(scores['weighted_cf_score'])} | {_number(scores['speech_category_score'])} | "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Experiment 011: Speech Evaluation.")
    ...
    parser.add_argument("--label", default="speech-evaluation")
...
def run(settings: Settings) -> int:
    ...
    output = experiment.directory / "speech"
    ...
    summary_document = {
        ...
        "inputs": {
            ...
            "speech_profile_source": "baseline.speech",
        },
        "patch": patch,
    ...
    write_text_exclusive(output / "speech_report.md", report)
    print(report)
    return 0
